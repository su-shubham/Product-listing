...
async def create_posts(posts:schemas.PostsCreate,db:Session=Depends(get_db)):
...
@router.put('/{id}')
async def update_posts(id:int,updates_post:schemas.PostsCreate,db:Session=Depends(get_db)):
